...
import { BanIcon, CreditCardIcon, Edit2Icon, RefreshCcwIcon, ArrowUpRightIcon, ArrowDownRightIcon, SearchIcon } from "lucide-react"

const transactions = [
  { id: 1, merchant: "Marriot Hotel Kigali", date: "Today, 10:30 AM", amount: 150000, type: "debit" },
  { id: 2, merchant: "Fatima Hotel", date: "Yesterday, 2:15 PM", amount: 300000, type: "debit" },
  { id: 3, merchant: "Amikus", date: "3 days ago", amount: 25000, type: "debit" },
  { id: 4, merchant: "Global Travel", date: "2 days ago", amount: 125000, type: "credit" },
]
            ...
            <CardHeader className="pb-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Search, filter, and sort this card&apos;s transactions.</p>
                </div>
                  ...
                  filteredTransactions.map((tx) => (
                    <div key={tx.id} className="flex items-center justify-between border-b border-slate-100 pb-4 last:border-0 last:pb-0">
                      ...
                      <div className={`font-semibold text-sm ${tx.type === 'debit' ? 'text-slate-900' : 'text-emerald-600'}`}>
                        {tx.type === 'debit' ? '-' : '+'}{cardData.currency} {tx.amount.toLocaleString()}
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-center text-sm text-slate-500">
                    No transactions match your search and filter choices.
                  </div>
                )}
              </div>
            </CardContent>
